...
- [ ] An unexpected failure on any screen shows a short message with a way back to the library, instead of a blank page
- [ ] A bank already in the library that no longer validates (for example after ticket 06 tightens validation) cannot take the app down: starting it explains the problem and offers to remove it
- [ ] Storage being unavailable (a private window that blocks IndexedDB, or a full disk) is explained in words a participant can act on, rather than failing silently
- [ ] Tests cover each of the three failures above
