...
src/
  ...
  music/       midi.ts  crab.ts  player.ts  background.ts   # the library's music, §8.6
...
scripts/
  ...
  crab-canon.ts                           # writes public/music/crab-canon.mid, §8.6
public/
  music/crab-canon.mid                    # generated by scripts/crab-canon.ts, committed
...
The library plays Bach's Crab Canon (The Musical Offering, BWV 1079, Canon 1 a 2) from `music/crab-canon.mid`, synthesised with Web Audio and looped. It plays on the library only, never during a quiz or on any other screen. Browsers allow sound only after the person has interacted with the page, so where autoplay is blocked it starts on the first click or key press. A Music toggle on the library turns it off, and the choice is remembered in the `settings` table. A missing file or a browser without Web Audio leaves the app silent and otherwise unchanged. The file is sequenced in this repo by `pnpm crab-canon`, which transcribes Bach's single line and writes the second voice as its exact retrograde. It is dedicated to the public domain under CC0 1.0. Ticket 15.
